...
//			}
			...
			{
				System.out.println("ALE BEFORE WE GET THE NONCANDIDATES-------------------------Looking into frequency of: "+this);
				...
				System.out.println("ALE Freq of this subgraph: "+freq);
				 finalFrequency= new IntFrequency(freq);
			}
			else
			{
				//ALE this is how we find the frequency of our subgraph code
				...
				if(Settings.CACHING)
				{
					//TODO zabat 2l non-candidates !!
					Decomposer<NodeType, EdgeType> com= new Decomposer<NodeType, EdgeType>(me);
					com.decompose(); //ALE this function removes edges and populates mappings attribute with them
					ArrayList<HashMap<HPListGraph<NodeType, EdgeType>, ArrayList<Integer>>> maps=com.getMappings();
					for (int i = 0; i < maps.size(); i++) //iterate over edges removed!! 
					{
						HashMap<HPListGraph<NodeType, EdgeType>, ArrayList<Integer>> edgeRemoved= maps.get(i);
				...
				df.searchExistances(); //ALE this is where SPpruner's getPrunedList() is called
				currentVariables=df.getResultVariables();
	 ...
	 */
	//ALE this is the function that is called from CanonicalPruningStep.java
...
//		}
	...
	{
		System.out.println("ALE, VARIABLES");
		for(int i = 0; i< currentVariables.length; i++)
		{
			System.out.println(currentVariables[i].toString());
		}
	}
	
	//ALE
	public void printSubgraphInstances()
	{
		// serialize graph
		String text = "";		
		
		BitSet nodes=  me.getNodes();
		BitSet edges= me.getEdges();
		for (int nodeIdx = nodes.nextSetBit(0); nodeIdx >= 0; nodeIdx = nodes.nextSetBit(nodeIdx + 1)) 
		{
			int label= (Integer)me.getNodeLabel(nodeIdx);
			text += "v " + nodeIdx + " "
			+ label + "\n";
		...
		{
			int node1=me.getNodeA(edgeIdx);
			int node2=me.getNodeB(edgeIdx);
			int edgeLabel=Integer.parseInt((String)me.getEdgeLabel(edgeIdx));
			if(me.getDirection(edgeIdx)>=0)
			{
				//ALE nodes are in order
				Variable a = currentVariables[node1];
				Variable b = currentVariables[node2];
				HashMap<Integer, myNode> pool = b.getList();
				for (Entry<Integer, myNode> entry : a.getList().entrySet())
				{
					//ALE children of this individual node
					//HashMap<Integer, ArrayList<MyPair<Integer, Double>>> children = entry.getValue().getReachableWithNodes();
					Set<Integer> childrenIDs = entry.getValue().getChildrenIDs();
					
					//System.out.println("Node " + entry.getKey() + " has children " + childrenIDs);
					
					//ALE do intersection
					Set<Integer> nodeBIDs = pool.keySet();
					//System.out.println("Pool is " + nodeBIDs);
					childrenIDs.retainAll(nodeBIDs);
					
					System.out.println(entry.getValue().getID() + " -> " + childrenIDs);
			...
			{
				//ALE nodes are in order
				Variable a = currentVariables[node2];
				Variable b = currentVariables[node1];
				HashMap<Integer, myNode> pool = b.getList();
				for (Entry<Integer, myNode> entry : a.getList().entrySet())
				{
					//ALE children of this individual node
					//HashMap<Integer, ArrayList<MyPair<Integer, Double>>> children = entry.getValue().getReachableWithNodes();
					Set<Integer> childrenIDs = entry.getValue().getReachableWithNodes().keySet();
