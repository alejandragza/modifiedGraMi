...
public class GSpanExtender<NodeType, EdgeType> extends
	...
	public Collection<SearchLatticeNode<NodeType, EdgeType>> getChildren(
		...
		children = new TreeSet<SearchLatticeNode<NodeType, EdgeType>>(); //ALE TreeSet in Java does not allow duplicate values
		dummy.clear();
		// start the run throu the chain for the given node
		first.call(node, dummy); //ALE start MiningStep (CanonicalPruningStep)
		return children;
	}
